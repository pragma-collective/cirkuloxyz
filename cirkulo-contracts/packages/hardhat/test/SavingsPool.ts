...
import { SavingsPool } from "../typechain-types";
...
describe("SavingsPool", function () {
  ...
  beforeEach(async function () {
    ...
    savingsPool = await SavingsPoolFactory.deploy(creator.address, circleId, circleName);
  });
  ...
  describe("Deposits", function () {
    ...
    it("Should allow member to deposit", async function () {
      ...
      await expect(savingsPool.connect(member1).deposit({ value: depositAmount })).to.changeEtherBalance(
        savingsPool,
        depositAmount,
      );
    });
    ...
    it("Should update member balance", async function () {
      const depositAmount = ethers.parseEther("1.0");
      await savingsPool.connect(member1).deposit({ value: depositAmount });
    ...
    it("Should emit Deposited event", async function () {
      const depositAmount = ethers.parseEther("1.0");
      await expect(savingsPool.connect(member1).deposit({ value: depositAmount }))
    ...
    it("Should allow multiple deposits", async function () {
      ...
      await savingsPool.connect(member1).deposit({ value: deposit1 });
      await savingsPool.connect(member1).deposit({ value: deposit2 });
    ...
    it("Should reject zero deposits", async function () {
      await expect(savingsPool.connect(member1).deposit({ value: 0 })).to.be.revertedWith("Must deposit something");
    });

    it("Should prevent non-members from depositing", async function () {
      await expect(savingsPool.connect(nonMember).deposit({ value: ethers.parseEther("1.0") })).to.be.revertedWith(
        "Not a member",
      );
    });
    ...
    it("Should prevent deposits when pool is not active", async function () {
      ...
      await expect(savingsPool.connect(member1).deposit({ value: ethers.parseEther("1.0") })).to.be.revertedWith(
        "Pool not active",
      );
    });
    ...
    it("Should track individual member balances separately", async function () {
      ...
      await savingsPool.connect(member1).deposit({ value: ethers.parseEther("1.0") });
      await savingsPool.connect(member2).deposit({ value: ethers.parseEther("2.0") });
  ...
  describe("Withdrawals", function () {
    beforeEach(async function () {
      await savingsPool.connect(creator).inviteMember(member1.address);
      await savingsPool.connect(member1).joinPool();
      await savingsPool.connect(member1).deposit({ value: ethers.parseEther("2.0") });
    });

    it("Should allow member to withdraw", async function () {
      const withdrawAmount = ethers.parseEther("1.0");
      await expect(savingsPool.connect(member1).withdraw(withdrawAmount)).to.changeEtherBalance(
        member1,
        withdrawAmount,
      );
    });
  ...
  describe("Goal Management", function () {
    ...
    it("Should return correct progress", async function () {
      ...
      await savingsPool.connect(creator).inviteMember(member1.address);
      await savingsPool.connect(member1).joinPool();
      await savingsPool.connect(member1).deposit({ value: ethers.parseEther("3.0") });
    ...
    it("Should check if goal is reached", async function () {
      ...
      await savingsPool.connect(creator).inviteMember(member1.address);
      await savingsPool.connect(member1).joinPool();
      await savingsPool.connect(member1).deposit({ value: ethers.parseEther("5.0") });
  ...
  describe("View Functions", function () {
    ...
    it("Should return member balance", async function () {
      await savingsPool.connect(member1).deposit({ value: ethers.parseEther("1.5") });
    ...
    it("Should return contract balance", async function () {
      ...
      expect(await savingsPool.getContractBalance()).to.equal(ethers.parseEther("3.0"));
    });
  });
});
